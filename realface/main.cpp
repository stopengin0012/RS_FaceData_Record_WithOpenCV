...
#pragma region VideoFileInit
		...
		recordCount++;
		startTime = newtime;
		videoname = "../moivefiles/" + startTime  + "_" + std::to_string(recordCount) + ".avi";
...
#pragma endregion ExpressionCSVFileInit
	...
	void initializeFace()
	{
		...
		*/
		...
		config->detection.maxTrackedFaces = 1;			//追加：一人のみ顔の取得を行う
		...
		config->QueryExpressions()->properties.maxTrackedFaces = 1;    //顔の表出情報の最大認識人数
		config->ApplyChanges();
...
#pragma region WriteCSVFiles
			...
			//Writing to CSV files about Landmark info
			//LandmarkPointのチェック
			bool isLandmarkOK = true;
			for (int s = 0; s < numPoints; s++) {	//Landmarksの画面対応の二次元位置情報
				if (&landmarkPoints[s] == nullptr) {
					isLandmarkOK = false;
				}
			}

			if (isLandmarkOK == false) {
				continue;
			}
